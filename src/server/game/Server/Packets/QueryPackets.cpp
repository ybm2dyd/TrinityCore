...
WorldPacket const* WorldPackets::Query::QueryCreatureResponse::Write()
{
    ...
    if (Allow)
    {
        ...
        _worldPacket << uint32(Stats.CreatureDisplay.size());
        _worldPacket << float(Stats.CreatureDisplayProbabilitySum);

        for (CreatureXDisplay const& display : Stats.CreatureDisplay)
        {
            _worldPacket << uint32(display.CreatureDisplayID);
            _worldPacket << float(display.DisplayScale);
